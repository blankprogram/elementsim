...
impl SandGame {
    ...
    fn try_move(&mut self, from: usize, to_x: usize, to_y: usize) -> bool {
        if let Some(to) = self.index(to_x, to_y) {
            let from_cell = self.grid[from];
            let target_cell = self.grid[to];
    
    
            let from_density = ELEMENT_DEFINITIONS.get(&from_cell.cell_type).unwrap().density;
            let target_density = ELEMENT_DEFINITIONS.get(&target_cell.cell_type).unwrap().density;
    
            if from_density > target_density {
                self.swap_and_update(from, to, to_x, to_y);
                return true;
            }
        }
    ...
    fn swap_and_update(&mut self, from: usize, to: usize, to_x: usize, to_y: usize) {
